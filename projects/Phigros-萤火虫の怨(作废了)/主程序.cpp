...
void init(void) {
	...
	midi.parse("midi/Phigros-萤火虫の怨.mid");
	...
	for(int i=0; i<noteNum; i++) {
		if(note[i].volume==0)
			continue;
		if(note[i].track==1) {
			track1[track1Num++]=note[i];
		} else if(note[i].track==2) {
			track2[track2Num++]=note[i];
		}
	}
}
...
void fun1(void) {
	auto makeParticle=[](int x,int z) {
		...
		useTemplate(
		    ...
		    String<const char*,int,const char*,int,const char*>("particles/fun1/fun1_",x,"_",z,"_1.json"),
		    String<const char*,int,const char*,int,const char*>("fun1_",x,"_",z,"_1"),
		    x/20.0,
		    Offset(
		...
		useTemplate(
		    String<const char*>("particles/template/[线]尖头.template"),
		    String<const char*,int,const char*,int,const char*>("particles/fun1/fun1_",x,"_",z,"_2.json"),
		    String<const char*,int,const char*,int,const char*>("fun1_",x,"_",z,"_2"),
		    x/20.0,
		    Offset(
		        String<int,const char*>(x,"*variable.emitter_age/variable.emitter_lifetime"),
		        String<double,const char*>(a,"*variable.emitter_age/variable.emitter_lifetime*(variable.emitter_age/variable.emitter_lifetime-1)"),
	...
	drawline(track1,initPoint,[&](Note a,Note b) {
		if(b.tick>=768)return;
		makeParticle(b.tick-a.tick,b.pitch-a.pitch);
		fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun1_%d_%d_1 %d %d %d\n",a.tick,b.tick-a.tick,b.pitch-a.pitch,a.tick,200,a.pitch);
		fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun1_%d_%d_2 %d %d %d\n",a.tick,b.tick-a.tick,b.pitch-a.pitch,a.tick,200,a.pitch);
	});
...
void fun3(void){
	useTemplate(
	    String<const char*>("particles/template/[线]通用.template"),
	    String<const char*>("particles/fun3/fun3_line1.json"),
	    String<const char*>("fun3_line1"),
	    50/20.0,
	    Offset(
	...
	useTemplate(
	    String<const char*>("particles/template/[线]通用.template"),
	    String<const char*>("particles/fun3/fun3_line2.json"),
	    String<const char*>("fun3_line2"),
	    50/20.0,
	    Offset(
	        String<int>(0),
	        String<int>(0),
	...
	fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun3_line1 %d %d %d\n",1152-5,1127,200,64);
	fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun3_line2 %d %d %d\n",1152-5,1152,200,39);
	
	bool flag=0;
	for(int i=1152,j=5;i<1536;i+=3,j+=(j<0?-1:1)){
		if((i-1152)%12==0){
			flag=!flag;
		}
		if(flag){
			fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun3_yellowButton_1 %d %d %d\n",i-20,1152+10,200,64+j*2);
			fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun3_beat %d %d %d\n",i,1152+10,200,64+j*2);
		}
		else{
			fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun3_yellowButton_2 %d %d %d\n",i-20,1152+j*2,200,64+10);
			fprintf(effect,"execute @p[scores={tick=%d}] ~~~ particle fun3_beat %d %d %d\n",i,1152+j*2,200,64+10);
		}
	}
}
void fun4(void){
