			...
			for(int idx=0; idx<getEventsNum(); idx++) {
				...
				if(allEvent[idx].second[0]==0xFF&&allEvent[idx].second[1]==0x51) {
					...
					bpmArr.push_back(BeatPerMinute {
						...
						allEvent[idx].first,
						idx==0?0:int(lastBeat+(allEvent[idx].second.getTime()-allEvent[idx-1].second.getTime())*(beatPerMinute/6e4)),
						idx==0?0:int(lastBar+(allEvent[idx].second.getTime()-allEvent[idx-1].second.getTime())*(beatPerMinute/6e4)/ts_nnn),
						beatPerMinute
					});
					...
					noteArr.push_back(Note {
						...
						instruments[allEvent[idx].second[0]&0xF]
					});
